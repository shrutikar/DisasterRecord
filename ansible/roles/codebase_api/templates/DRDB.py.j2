...
class DRDB:
  ...
  def create_detectedobjects_table(self):
    cmd="CREATE TABLE IF NOT EXISTS detectedobjects (\
          id integer PRIMARY KEY,\
          campaignid text NOT NULL,\
  ...
  def clear_detectedobjects_table(self):
    cmd="DELETE FROM detectedobjects;"
    try:
      self._exeCreate(cmd)
      return True
    except:
